 ...
 * freestanding firmware build defines LCA_MMIO_ACCEL and dispatches each
 * Keccak-f[1600] permutation to the 24-cycle hardware engine.
 *
 ...
 **************************************************/
static void KeccakF1600_StatePermute_sw(uint64_t *state) {
    int round;
...
#ifdef LCA_MMIO_ACCEL
#define LCA_KECCAK_CTRL  (*(volatile uint32_t *)0x20001000u)
#define LCA_KECCAK_STATE ((volatile uint32_t *)0x20001100u)

static void KeccakF1600_StatePermute(uint64_t *state) {
    size_t i;

    for (i = 0; i < 25; ++i) {
        LCA_KECCAK_STATE[2 * i] = (uint32_t)state[i];
        LCA_KECCAK_STATE[2 * i + 1] = (uint32_t)(state[i] >> 32);
    }
    LCA_KECCAK_CTRL = 1u;
    while ((LCA_KECCAK_CTRL & 1u) != 0u) {
        /* The peripheral completes one round per clock. */
    }
    for (i = 0; i < 25; ++i) {
        state[i] = (uint64_t)LCA_KECCAK_STATE[2 * i] |
                   ((uint64_t)LCA_KECCAK_STATE[2 * i + 1] << 32);
    }
}
#else
static void KeccakF1600_StatePermute(uint64_t *state) {
